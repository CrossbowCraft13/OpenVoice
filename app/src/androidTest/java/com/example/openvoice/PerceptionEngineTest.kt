...
import com.example.openvoice.perception.*
...
class PerceptionEngineTest {
    ...
    fun router_routesVisionOnlyWhenA11yUnavailable() {
        ...
        val result = com.example.openvoice.intent.IntentResult("READ_SCREEN", 0.9f)
        ...
        assertTrue("READ_SCREEN should route to Vision or Accessibility",
            resolution is com.example.openvoice.router.Resolution.Vision ||
            resolution is com.example.openvoice.router.Resolution.Accessibility)
    }
    ...
    fun router_routesA11yFirst_whenAvailable() {
        ...
        assertFalse("READ_SCREEN should not be unsupported",
            resolution is com.example.openvoice.router.Resolution.Unsupported)
    }

    @Test
    fun router_nativeIntents_stillFastPath() {
        val result = com.example.openvoice.intent.IntentResult("SET_TIMER", 0.95f, mapOf("duration" to "30 seconds"))
        val resolution = router.resolve(result)
        assertTrue("SET_TIMER should be native", resolution is com.example.openvoice.router.Resolution.Native)
    }
